...
chrome.contextMenus.onClicked.addListener(info => {
	if (info.menuItemId == 'add-sticky-note') {
		chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
			if (tabs.length != 'undefined' && tabs.length == 1) var currentURL = tabs[0].url;

			if (typeof currentURL != 'undefined') console.log(currentURL);
...
chrome.storage.onChanged.addListener((changes, namespace) => {
	for (var key in changes) {
		...
		console.log(
			`Storage key ${key} in namespace ${namespace} changed. Old value was ${storageChange.oldValue}, new value is ${storageChange.newValue}.`
		);
	}
});
	...
	switch (request.action) {
		...
		case 'remove-note':
			chrome.storage.sync.get('Notes', items => {
				...
				NoteList = NoteList.filter(it => parseInt(it.id) !== request.id);

				console.log(NoteList);

				chrome.storage.sync.set({ Notes: NoteList }, () => {});

				var element = document.querySelector(`#stickyNote-${request.id}`);
		...
		case 'load-notes':
			chrome.storage.sync.get('Notes', items => {
				...
				chrome.storage.sync.get('maxID', item => {
					const id = item.maxId;
					sendResponse({ maxID: id, note_list: list });
				});
			});
